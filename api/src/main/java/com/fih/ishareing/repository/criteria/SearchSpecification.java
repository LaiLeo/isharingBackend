...
public class SearchSpecification<T> implements Specification<T> {
	...
	private static final long serialVersionUID = 7635600632466454398L;

	public enum OPERARATOR {
		EQ("="), NE("!="), GT(">"), GTE(">="), LT("<"), LTE("<="), LIKE("like"), BETWEEN("between"), IN("in"),
		NIN("not in");
	...
	public SearchSpecification(SearchCriteria searchCriteria, Class<T> entityClass, CriteriaFilterCondition filter) {
		...
		switch (OPERARATOR.valueOf(criteria.getOperation().toUpperCase())) {
			case IN:
				if (StringToPrimitypeUtil.isSplitByCommaString(value)) {
					return root.get(key).in(StringToPrimitypeUtil.toObjects(value));
				}
			case NIN:
				if (StringToPrimitypeUtil.isSplitByCommaString(value)) {
					return root.get(key).in(StringToPrimitypeUtil.toObjects(value)).not();
				}
			case EQ:
			...
			case LIKE:
				if (root.get(key).getJavaType() == String.class) {
					...
					throw new ParameterException("Operator is not supported. [" + criteria.getOperation()
							+ " only can be used on string type]");
				}
			case BETWEEN:
				String[] split = value.split(",");
				if (split.length != 2) {
					throw new ParameterException(
							key + " : the value of BETWEEN operator only can be split by ',' into two values.");
				}
				if (Date.class.isAssignableFrom(root.get(key).getJavaType())) {
					if (split.length != 2) {
						throw new ParameterException(key
								+ " is Date type, the value of BETWEEN operator only can be split by ',' into two Dates with format 'yyyy-MM-dd' or 'yyyy-MM-dd HH:mm:ss'");
					}
					...
					if (!v1.getClass().equals(v2.getClass())) {
						throw new ParameterException(
								key + " : the data type of value after BETWEEN operator must be the same.");
					}
					if (root.get(key).getJavaType() == Integer.class && v1 instanceof Integer) {
						...
						throw new ParameterException(key + " between " + value
								+ " is invalid. BETWEEN operator only accpet Integer, Double, String, Date type.");
					}
				}
			default:
		}
